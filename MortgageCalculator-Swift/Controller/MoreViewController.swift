...
class MoreViewController: UITableViewController {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if (indexPath as NSIndexPath).row == 5 {
            baseCell.titleLabel?.text = "清除缓存"
            baseCell.detaileLabel?.text = String(self.fileSizeOfCache()) + "  M"
        }
    ...
    func zanImageViewTap(){
        UIApplication.shared.open(URL(string:"http://itunes.apple.com/WebObjects/MZStore.woa/wa/viewContentsUserReviews?id=1106215431&pageNumber=0&sortOrdering=2&type=Purple+Software&mt=8")!, options: [:], completionHandler: nil)
    }
    func tuImageView(){
